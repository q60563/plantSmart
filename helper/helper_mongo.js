...
var mongoLink = 'mongodb://localhost:27017/plant'; 
...
var find = function(collectionName, query, option) {
    return new Promise(function(resolve, reject) {
        MongoClient.connect(mongoLink, function(err, db) {
            ...
            if(option==undefined || option==null)
            {
                collection.find(query).sort({createAt: -1}).limit(10).toArray(function(err, res) {
                    db.close();
                    if (err) reject(err);
                    else resolve(res);
                });
            }else{
                collection.find(query, option).sort({createAt: -1}).limit(10).toArray(function(err, res) {
                    db.close();
                    if (err) reject(err);
                    else resolve(res);
                });
